...
# #This thing below, i dont need as i already have my expert traj // they are my input 
...
#     scaler.fit(self.vectorized_demonstrations) # take my exp dem as intput 
#     return scaler


# def reset(self): #only needs exp demonstrations as input 
...
# #see in the imitation loop of google brain what obs_act is 
# # use action and corresponding state --> has to be added within the imitation loop before walling pwil rewards 
...
# #give the function below an inpput that is the concatenated ans sample expert trajectory
...
def get_scaler(expert_demonstrations, agent_rollout):
    """Defines a scaler to derive the standardized Euclidean distance."""
    scaler = preprocessing.StandardScaler()  
    scaler.fit(expert_demonstrations)
    scaler =  scaler

        
    #"""Makes all expert transitions available and initialize weights.""" 
    expert_atoms = copy.deepcopy(scaler.transform(expert_demonstrations))
    num_expert_atoms = len(expert_atoms)
    expert_weights = np.ones(num_expert_atoms) / (num_expert_atoms)
    
    
    #concatenated_exp_traj = concatenate_and_sample(expert_demonstrations)



    trajectory = scaler.transform(agent_rollout)
    trajectory_weights = 1./len(trajectory) * np.ones(len(trajectory))
    
    cost_matrix = ot.dist(trajectory, expert_atoms, metric='euclidean')
    
    w2_dist = ot.emd2(trajectory_weights, expert_weights, cost_matrix)
